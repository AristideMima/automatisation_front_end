...
import { urlActivate, setActiveFile, deleteFile } from '../../constants/constants'
...
class TemplateActive extends Component {
    ...
    sendActivatation = (obj) => {
        ...
        if (typeObj === "delete"){
            ...
            axios.post(`${deleteFile}`, {"id": id, "type": type}, config)
                .then( res => {
            ...
            axios.post(`${setActiveFile}`, {"type_file": type, "id": obj.id}, config)
                .then( res => {
                    this.setState({data: data})
                    notification.success({
